  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-5 items-center">
                <label htmlFor="" className="flex flex-col gap-2 w-full" >
                    ...
                    <Select>
                        ...
                        <SelectContent defaultValue={"house"} className="bg-dialog-color">
                            <SelectGroup
                                className="bg-dialog-color"
                                defaultValue={data?.type ?? "house"}
                                onChange={(e) =>
                                    updateData!({
                                        ...data,
                                        type: PropertySchema.shape.type.parse(
            ...
            <div className="flex justify-between">
                <label htmlFor="has-parking" className="flex gap-2  items-center">
                    <p>HasParking</p>
                    <Switch id="has-parking" className="cursor-pointer bg-button data-[state=checked]:bg-button " checked={data?.hasParking}
                    onChange={e => updateData!({...data , hasParking : Boolean(e.currentTarget.value)})}  />
                </label>
                <label htmlFor="IsFurnished" className="flex gap-2  items-center">
                    <p>IsFurnished</p>
                    <Switch id="IsFurnished" className="cursor-pointer bg-button data-[state=checked]:bg-button " checked={data?.isFurnished}
                    onChange={e => updateData!({...data , isFurnished : Boolean(e.currentTarget.value)})}  />
                </label>
                {/* <label htmlFor="IsActive" className="flex gap-2  items-center">
                    <p>IsActive</p>
                    <Switch id="IsActive" className="cursor-pointer bg-button data-[state=checked]:bg-button " checked={data?.isActive}
                    onChange={e => updateData!({...data , isActive : Boolean(e.currentTarget.value)})}  />
                </label> */}
                <label htmlFor="IsForRent" className="flex gap-2  items-center">
                    <p>IsForRent</p>
                    <Switch id="IsForRent" className="cursor-pointer bg-button data-[state=checked]:bg-button " checked={data?.isForRent}
                    onChange={e => updateData!({...data , isForRent : Boolean(e.currentTarget.value)})}  />
                </label>
                <label htmlFor="IsForSale" className="flex gap-2  items-center">
                    <p>IsForSale</p>
                    <Switch id="IsForSale" className="cursor-pointer bg-button data-[state=checked]:bg-button " checked={data?.isForSale}
                    onChange={e => updateData!({...data , isForSale : Boolean(e.currentTarget.value)})}  />
                </label>
            </div>
        ...
        <div className="w-full">
            <button
                onClick={nextStep}
                className="bg-button w-full rounded-md p-2 text-md"
            >
                Continue
            </button>
        </div>
